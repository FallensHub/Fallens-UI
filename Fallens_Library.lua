...
local Compkiller = {
	...
	Windows = {},
	Scale = {
		Window = UDim2.new(0, 380,0, 460),
		Mobile = UDim2.new(0, 440,0, 365),
		Small = UDim2.new(0, 380,0, 320),
		PC = UDim2.new(0, 380,0, 460),
		Min = UDim2.new(0, 380,0, 320),
		Max = UDim2.new(0, 900,0, 800),
		TabOpen = 185,
		TabClose = 85,
	},
...
function Compkiller:_Resize(InputFrame: Frame, TargetFrame: Frame, Speed: number, MinSize: UDim2, MaxSize: UDim2)
	local resizeToggle: boolean = false;
	local dragStart: Vector3 = nil;
	local startSize: UDim2 = nil;
	local Tween = TweenInfo.new(Speed);

	MinSize = MinSize or Compkiller.Scale.Min;
	MaxSize = MaxSize or Compkiller.Scale.Max;
	...
	local updateInput = function(input)
		...
		local newX = clamp(startSize.X.Offset + delta.X, MinSize.X.Offset, MaxSize.X.Offset);
		local newY = clamp(startSize.Y.Offset + delta.Y, MinSize.Y.Offset, MaxSize.Y.Offset);

		Compkiller:_Animation(TargetFrame,Tween,{
	...
	UserInputService.InputChanged:Connect(function(input)
		if resizeToggle and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			updateInput(input);
		end;
	end);
...
function Compkiller:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal)
	...
	elseif Name == "CheckBox" then
		...
		CheckBox.Name = Compkiller:_RandomString()
		CheckBox.Parent = LinkValues
		CheckBox.BackgroundColor3 = Compkiller.Colors.DropColor
		CheckBox.BorderColor3 = Color3.fromRGB(0, 0, 0)
		CheckBox.BorderSizePixel = 0
		CheckBox.Size = UDim2.new(0, 16, 0, 16)
		CheckBox.ZIndex = GlobalBlock.ZIndex + 1
		CheckBox.LayoutOrder = -#LinkValues:GetChildren();
		...
		CheckIcon.Name = Compkiller:_RandomString()
		...
		CheckIcon.ZIndex = GlobalBlock.ZIndex + 2
		CheckIcon.Image = Compkiller:_GetIcon("lucide-check")
		CheckIcon.ImageColor3 = Compkiller.Colors.SwitchColor
		CheckIcon.ImageTransparency = 1
		CheckIcon.ScaleType = Enum.ScaleType.Fit

		local CheckElement = function(bool,noChange)
			if not noChange then
				Default = bool;
			end;
			...
			if bool then
				...
				Compkiller:_Animation(CheckIcon,rep.Tween,{
					ImageTransparency = 0
				})

				Compkiller:_Animation(CheckBox,rep.Tween,{
					BackgroundColor3 = Compkiller.Colors.Toggle
				})
			else
				CheckBox:SetAttribute('Enabled',false);
		...
		local Input = Compkiller:_Input(CheckBox);

		Compkiller:_Hover(Input , function()
			if not Default then
				Compkiller:_Animation(UIStroke,rep.Tween,{
					Color = Compkiller.Colors.Toggle
				})
			end;
		end , function()
			Compkiller:_Animation(UIStroke,rep.Tween,{
				Color = Compkiller.Colors.HighStrokeColor
			})
		end);

		local CheckUI = function(bool)
			if bool then
				CheckElement(Default,true);
		...
		CheckElement(Default);

		Signal:Connect(CheckUI)

		return {
			Root = CheckBox,
			ChangeValue = CheckElement,
			Input = Input,
			ToggleUI = CheckUI,
		};
...
function Compkiller:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal , DisableStackKeybind)
	...
	function Args:AddCheckBox(Config : Toggle)
		Config = Compkiller.__CONFIG(Config,{
			Name = "CheckBox",
			Default = false,
			Flag = nil,
		...
		local CheckBox = Block:AddLink('CheckBox' , Config.Default);

		CheckBox.Input.MouseButton1Click:Connect(function()
			Config.Default = not Config.Default;

			CheckBox.ChangeValue(Config.Default);
...
function Compkiller.new(Config : Window)
	...
	local CurrentScale = Config.Scale; -- tracks the user-selected scale so it survives close/reopen
	local WindowArgs = {
		SelectedTab = nil,
		Tabs = {},
		LastTab = nil,
	...
	WindowOpen:Connect(function(v)
		...
		if v then
			...
			Compkiller:_Animation(MainFrame,TweenInfo.new(0.2),{
				Size = UDim2.new(math.max(CurrentScale.X.Scale - 0.05,0) , CurrentScale.X.Offset - 10 , math.max(CurrentScale.Y.Scale - 0.05,0) , CurrentScale.Y.Offset - 10)
			})
	...
	do
		...
		SectionFrame.ScrollBarImageColor3 = Compkiller.Colors.Highlight
		SectionFrame.TopImage = ""
		SectionFrame.BottomImage = ""
		...
		function WindowArgs.UserSettings:Create()
			...
			local MAX_USERSETTINGS_HEIGHT = 300; -- panel stops growing here; content beyond this scrolls

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				local ContentHeight = UIListLayout.AbsoluteContentSize.Y;
				local PanelHeight = math.min(ContentHeight + 50, MAX_USERSETTINGS_HEIGHT);

				Compkiller:_Animation(UserSettings,TweenInfo.new(0.2),{
					Size = UDim2.new(0, 235, 0, PanelHeight)
				})

				SectionFrame.CanvasSize = UDim2.new(0, 0, 0, ContentHeight + 10);
			end);
			...
			local SettingsAPI = Compkiller:_LoadElement(SectionFrame , true , Signal);

			local ScalePresets = {
				["Small"] = Compkiller.Scale.Small,
				["Mobile"] = Compkiller.Scale.Mobile,
				["PC"] = Compkiller.Scale.PC,
			};

			local CurrentScaleName = (Compkiller:_IsMobile() and "Mobile") or "Small";

			SettingsAPI:AddDropdown({
				Name = "Menu Scale",
				Default = CurrentScaleName,
				Values = {"Small","Mobile","PC"},
				Multi = false,
				Callback = function(Value : string)
					local NewScale = ScalePresets[Value];

					if not NewScale then return; end;

					CurrentScale = NewScale; -- remember choice so it survives close/reopen

					Compkiller:_Animation(MainFrame,TweenInfo.new(0.35,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
						Size = NewScale
					});
				end;
			});
	...
	WindowArgs.LOOP_THREAD = task.spawn(function()
		...
		ResizeButton.Name = Compkiller:_RandomString()
		...
		ResizeButton.Image = Compkiller:_GetIcon("move-diagonal-2")
		ResizeButton.ImageColor3 = Compkiller.Colors.SwitchColor
		ResizeButton.ImageTransparency = 0.45
		ResizeButton.Active = true

		table.insert(Compkiller.Elements.SwitchColor,{
			Element = ResizeButton,
			Property = 'ImageColor3'
		});

		Compkiller:_Hover(ResizeButton,function()
			Compkiller:_Animation(ResizeButton,TweenInfo.new(0.15),{
				ImageTransparency = 0
			});
		end,function()
			Compkiller:_Animation(ResizeButton,TweenInfo.new(0.15),{
		...
		Compkiller:_AddDragBlacklist(ResizeButton);

		Compkiller:_Resize(ResizeButton , MainFrame , 0.05 , Compkiller.Scale.Min , Compkiller.Scale.Max);
