--[[
    ...
    Author: 4lpaca
    License: MIT
    Github: https://github.com/4lpaca-pin/Fallens
    Original: Fallens.net

        Version: 2.6
    - Improved Performance Mode
    	Fallens:OptimizeMode(< boolean >)
  	- Added Auto Color Logo
		Fallens:CustomIconHighlight()
--]]
...
Fallens.DragBlacklist = {};
Fallens.IaDrag = false;
Fallens.LastDrag = tick();
Fallens.Flags = {};
...
function Fallens:OptimizeMode(v)
	Fallens.PerformanceMode = v;
end;

function Fallens:IsStudio()
	return RunService:IsStudio()	
end;

function Fallens:CustomIconHighlight()
	Fallens.CustomHighlightMode = true;
end;

function Fallens:_SetNilP(Ins: Instance , Parent: Instance)
	Fallens.WindowsNil = Fallens.WindowsNil or {};
	Fallens.NilFolder = Fallens.NilFolder or Instance.new('Folder');

	if not Fallens.WindowsNil[Ins] then
		local win = Fallens:_GetWindowFromElement(Ins);

		Fallens.WindowsNil[Ins] = win;
	end;

	Ins.Parent = Parent or Fallens.NilFolder;
end;

function Fallens:SetAllText(flags : {[string] : string})
	if not flags then -- reset to default
		for i,v in next , Fallens.Flags do
			if v.SetText then
				v:SetText(nil);
			end;
		end;
	...
	for i,v in next , flags do
		if Fallens.Flags[i] and Fallens.Flags[i].SetText then
			Fallens.Flags[i]:SetText(v);
		end;
	end;
end;

function Fallens:_GetIcon(name : string , font_aws) : string
	if Fallens.SecureMode then
		local AssetId;

		if font_aws then
			AssetId = Fallens.FontAwesome[name] or name;
		else
			AssetId = Fallens.Lucide['lucide-'..tostring(name)] or Fallens.Lucide[name] or Fallens.Lucide[tostring(name)] or Fallens.FontAwesome[name] or name;
		end;

		if AssetId and AssetId ~= nil then
			local asset = Fallens:CacheImage(AssetId);
	...
	if font_aws then
		return Fallens.FontAwesome[name] or name;
	end;

	return Fallens.Lucide['lucide-'..tostring(name)] or Fallens.Lucide[name] or Fallens.Lucide[tostring(name)] or Fallens.FontAwesome[name] or name;
end;

function Fallens:_RandomString() : string
...
function Fallens:_IsMouseOverFrame(Frame : Frame) : boolean
	if not Frame then
		return;
	end;
...
function Fallens:_Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function Fallens:_Animation(Self: Instance , Info: TweenInfo , Property :{[K] : V})
...
function Fallens:_Input(Frame : Frame , Callback : () -> ()) : TextButton
...
function Fallens:GetCalculatePosition(planePos: number, planeNormal: number, rayOrigin: number, rayDirection: number) : number
	local n = planeNormal;
...
function Fallens:_Blur(element : Frame , WindowRemote) : RBXScriptSignal
	if Fallens.SecureMode and not Fallens.SecurityConfig.BlurEnabled then
		return game.Changed:Connect(function() end);
	end;

	local Part = Instance.new('Part',Fallens.ArcylicParent);
	...
	Part.CanQuery = false;
	Part.CollisionGroup = Fallens:_RandomString();
	...
	DepthOfField.Name = Fallens:_RandomString();

	Part.Name = Fallens:_RandomString();
	...
	local UpdateFunction = function()
		...
		if IsWindowActive then
			...
			Fallens:_Animation(DepthOfField,TweenInfo.new(0.1),{
				NearIntensity = 1
			})

			Fallens:_Animation(Part,TweenInfo.new(0.1),{
				Transparency = 0.97,
		...
		if IsWindowActive then
			...
			local pos0 = Fallens:GetCalculatePosition(planeOrigin, planeNormal, ray0.Origin, ray0.Direction);
			local pos1 = Fallens:GetCalculatePosition(planeOrigin, planeNormal, ray1.Origin, ray1.Direction);
...
function Fallens:_AddDragBlacklist(Frame: Frame)
	local IsAdded = false;
	local BASE_TIME = 0.01;

	local SET_BLACKLIST = function(value)
		local index = table.find(Fallens.DragBlacklist , Frame);

		if value and not Fallens.IS_DRAG_MOVE then
			if not index then
				table.insert(Fallens.DragBlacklist,Frame);
			end;
		else
			if index then
				table.remove(Fallens.DragBlacklist,index);
			end;
		end;
	end;

	Frame.InputBegan:Connect(function(input)
		if Fallens:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(true)
		end;
	end);
	...
	UserInputService.InputChanged:Connect(function()
		if not Fallens:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(false);
		end
	end);
end;

function Fallens:_GetWindowFromElement(Element)
	if Fallens.WindowsNil[Element] then
		return Fallens.WindowsNil[Element];
	end;

	for i,v : ScreenGui in next , Fallens.Windows do
		if v and Element:IsDescendantOf(v) then
			return v;
		end;
	end;

	for Frame,Window in next , Fallens.WindowsNil do
...
function Fallens.__SIGNAL(default)
...
function Fallens:_Hover(Frame: Frame , OnHover: () -> any?, Release: () -> any?)
	Frame.MouseEnter:Connect(OnHover);

	Frame.MouseLeave:Connect(Release);
end;

function Fallens.__CONFIG(config , default)
	config = config or {};
...
function Fallens:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local updateInput = function(input)
		...
		Fallens:_Animation(MoveFrame,Tween,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #Fallens.DragBlacklist <= 0 then 
			dragToggle = true
			...
			input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					dragToggle = false;
					Fallens.IS_DRAG_MOVE = false;
				end
			end)
		end

		if not Fallens.IsDrage and dragToggle then
			Fallens.LastDrag = tick();
		end;

		Fallens.IaDrag = dragToggle;
	end)

	UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch and #Fallens.DragBlacklist <= 0 then
			if dragToggle then
				Fallens.IS_DRAG_MOVE = true;
				updateInput(input)
			else
				Fallens.IS_DRAG_MOVE = false;
			end
		else
			if #Fallens.DragBlacklist > 0 then
				dragToggle = false
				Fallens.IS_DRAG_MOVE = false;
			end
		end
...
function Fallens:_IsMobile()
	return UserInputService.TouchEnabled;
end;

function Fallens:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal)
	if Name == "Toggle" then
		...
		Toggle.Name = Fallens:_RandomString()
		Toggle.Parent = LinkValues
		Toggle.BackgroundColor3 = Fallens.Colors.DropColor
		...
		table.insert(Fallens.Elements.DropColor , {
			Element = Toggle,
		...
		UIStroke.Color = Fallens.Colors.HighStrokeColor
		UIStroke.Parent = Toggle

		table.insert(Fallens.Elements.HighStrokeColor,{
			Element = UIStroke,
			Property = "Color"
		})

		ToggleValue.Name = Fallens:_RandomString()
		ToggleValue.Parent = Toggle
		ToggleValue.AnchorPoint = Vector2.new(0.5, 0.5)
		ToggleValue.BackgroundColor3 = Fallens.Colors.SwitchColor
		...
		local ToggleElement = function(bool,noChange)
			...
			if bool then
				...
				Fallens:_Animation(ToggleValue,rep.Tween,{
					Position = UDim2.new(0.75, 0, 0.5, 0)
				})

				Fallens:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = Fallens.Colors.Toggle
				})
			else
				...
				Fallens:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = Fallens.Colors.DropColor
				})
			end;
		end;

		local Input = Fallens:_Input(Toggle);

		Fallens:_Hover(Input , function()
			if not Default then
				Fallens:_Animation(ToggleValue,rep.Tween,{
		...
		local ToggleUI = function(bool)
			if bool then
				...
				Fallens:_Animation(UIStroke,rep.Tween,{
					Transparency = 0
				})
			else
	...
	elseif Name == "ColorPicker" then
		...
		ColorPicker.Name = Fallens:_RandomString()
		...
		ColorFrame.Name = Fallens:_RandomString()
		...
		Signal:Connect(function(bool)
			if bool then
				Fallens:_Animation(ColorFrame,TweenInfo.new(0.15),{
					BackgroundTransparency = 0,
				})

				Fallens:_Animation(UIStroke,TweenInfo.new(0.15),{
					Transparency = 0,
				})
			else
				Fallens:_Animation(ColorFrame,TweenInfo.new(0.15),{
		...
		Fallens:_Hover(ColorPicker, function()
			if Signal:GetValue() then
				Fallens:_Animation(UIScale,TweenInfo.new(0.35),{
					Scale = 1.2
				})
			end;
		end , function()
	...
	elseif Name == "Keybind" then
		...
		Keybind.Name = Fallens:_RandomString()
		Keybind.Parent = LinkValues
		Keybind.BackgroundColor3 = Fallens.Colors.DropColor
		...
		TextLabel.TextColor3 = Fallens.Colors.SwitchColor
		TextLabel.TextSize = 12.000
		TextLabel.TextTransparency = 0.200

		table.insert(Fallens.Elements.SwitchColor,{
			Element = TextLabel,
			Property = "TextColor3"
		});
		...
		local Update = function()
			...
			Fallens:_Animation(Keybind,TweenInfo.new(0.1),{
		...
		local ToggleUI = function(bool)
			if bool then
				Fallens:_Animation(Keybind,rep.Tween,{
				...
				Fallens:_Animation(TextLabel,rep.Tween,{
	...
	elseif Name == "Helper" then
		...
		InfoButton.Name = Fallens:_RandomString()
		...
		InfoButton.Image = Fallens:CacheImage("rbxassetid://10723415903")
		...
		BlockText.BackgroundColor3 = Fallens.Colors.BlockColor
		...
		BlockText.Text = " "
		BlockText.TextColor3 = Fallens.Colors.SwitchColor
		...
		UIStroke.Color = Fallens.Colors.StrokeColor
		...
		Signal:Connect(function(bool)
			if bool then
				Fallens:_Animation(InfoButton,TweenInfo.new(0.15),{
		...
		Fallens:_Hover(InfoButton, function()
			if Signal:GetValue() then
				Fallens:_Animation(InfoButton,TweenInfo.new(0.15),{
	...
	elseif Name == "Option" then
		...
		OptionButton.Name = Fallens:_RandomString()
		...
		OptionButton.Image = Fallens:CacheImage("rbxassetid://14007344336")
		...
		Signal:Connect(function(bool)
			if bool then
				Fallens:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.500
				})
			else
				Fallens:_Animation(OptionButton,TweenInfo.new(0.15),{
		...
		Fallens:_Hover(OptionButton, function()
			if Signal:GetValue() then
				Fallens:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.1
				})
...
function Fallens:_CreateBlock(Signal)
	...
	if Fallens:_IsMobile() then
		Fallens:_AddDragBlacklist(GlobalBlock);
	end;
	...
	LinkValues.Name = Fallens:_RandomString()
	...
	BlockLine.Name = Fallens:_RandomString()
	BlockLine.Parent = GlobalBlock
	BlockLine.AnchorPoint = Vector2.new(0.5, 1)
	BlockLine.BackgroundColor3 = Fallens.Colors.LineColor
	...
	table.insert(Fallens.Elements.LineColor,{
		Element = BlockLine,
		Property = "BackgroundColor3"
	});
	...
	function rep:SetTextColor(Color)
		local oldIndex = table.find(Fallens.Elements.SwitchColor , BlockText);

		table.remove(Fallens.Elements.SwitchColor , oldIndex);

		BlockText.TextColor3 = Color;

		table.insert(Fallens.Elements.Risky , {
			Element = BlockText,
			Property = 'TextColor3'
		});
	...
	function rep:SetTransparency(num)
		...
		Fallens:_Animation(BlockText,TweenInfo.new(0.3),{
	...
	function rep:SetVisible(bool)
		if bool then
			Fallens:_Animation(BlockText,rep.Tween,{
				TextTransparency = rep.TextTransparency
			});

			Fallens:_Animation(BlockLine,rep.Tween,{
	...
	function rep:AddLink(Name , Default)
		return Fallens:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal);
	end;

	return rep;
end;

Fallens.Hash = function(str: string)
...
function Fallens:CacheImage(id: string) : string
	if not Fallens.SecureMode or not id or not id:byte() then
		return id or "";
	end;

	assert(Fallens.SecureMode , "please use Fallens:Security(< string >) before cache image")
	assert(Fallens.CacheDirectory , "please use Fallens:Security(< string >) before cache image")
	...
	local Hash = Fallens.Hash(id);

	local cache_path = string.format("%s/cache-%s.png" ,Fallens.CacheDirectory , Hash);

	if isfile(cache_path) then
		return (getcustomasset or getsynasset or function() return ''; end)(cache_path);
	end;

	local imgSize = Fallens.SecurityConfig.ImageScale;
...
function Fallens:Security(directory: string,Config: SecurityConfig) -- Security Mode
	directory = directory or "Fallens-Cache";

	if not isfolder(directory) then
		makefolder(directory);
	end;

	Fallens.SecureMode = true;

	Fallens.SecurityConfig = Config or {};

	Fallens.CacheDirectory = directory;
end;

function Fallens:_AddColorPickerPanel(Button: ImageButton , Callback: (Color: Color3) -> any?)
	local Window = Fallens:_GetWindowFromElement(Button);
	...
	ColorPickerWindow.Name = Fallens:_RandomString()
	ColorPickerWindow.Parent = Window
	ColorPickerWindow.BackgroundColor3 = Fallens.Colors.BlockBackground
	...
	table.insert(Fallens.Elements.BlockBackground,{
		Element = ColorPickerWindow,
		Property = "BackgroundColor3"
	});

	Fallens:_AddDragBlacklist(ColorPickerWindow)
	...
	ColorPickBox.Name = Fallens:_RandomString()
	...
	ColorPickBox.Image = Fallens:CacheImage("http://www.roblox.com/asset/?id=112554223509763");

	MouseMovement.Name = Fallens:_RandomString()
	...
	MouseMovement.Image = Fallens:CacheImage("rbxassetid://4805639000")
	...
	ColorRedGreenBlue.Name = Fallens:_RandomString()
	...
	ColorRGBSlide.Name = Fallens:_RandomString()
	...
	Left.Name = Fallens:_RandomString()
	...
	Right.Name = Fallens:_RandomString()
	...
	ColorOpc.Name = Fallens:_RandomString()
	...
	ColorOptSlide.Name = Fallens:_RandomString()
	...
	Left_2.Name = Fallens:_RandomString()
	...
	Right_2.Name = Fallens:_RandomString()
	...
	TransparentImage.Name = Fallens:_RandomString()
	...
	TransparentImage.Image = Fallens:CacheImage("rbxassetid://6198493000")
	...
	HexFrame.Name = Fallens:_RandomString()
	HexFrame.Parent = ColorPickerWindow
	HexFrame.AnchorPoint = Vector2.new(0.5, 1)
	HexFrame.BackgroundColor3 = Fallens.Colors.BlockColor
	...
	UIStroke_8.Color = Fallens.Colors.HighStrokeColor
	...
	Fallens:_AddPropertyEvent(ColorPickerWindow,function(v)
		ColorPickerWindow.Visible = v;

		if Fallens.PerformanceMode then
			if ColorPickerWindow.Visible then
				Fallens:_SetNilP(ColorPickerWindow , Window);
			else
				Fallens:_SetNilP(ColorPickerWindow , nil);
			end;
		else
			Fallens:_SetNilP(ColorPickerWindow , Window);
		end;
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			Fallens:_Animation(ColorPickerWindow,Tween2,{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 175, 0, 200)
			});

			Fallens:_Animation(ColorPickerWindow,Tween,{
				Position = MainPosition,
			});

			Fallens:_Animation(UIStroke_8,Tween,{
				Transparency = 0
			});

			Fallens:_Animation(UIStroke_7,Tween,{
				Transparency = 0.5
			});

			Fallens:_Animation(UIStroke_6,Tween,{
				Transparency = 0
			});

			Fallens:_Animation(UIStroke_5,Tween,{
				Transparency = 0
			});

			Fallens:_Animation(UIStroke_4,Tween,{
				Transparency = 0
			});

			Fallens:_Animation(UIStroke_3,Tween,{
				Transparency = 0
			});

			Fallens:_Animation(UIStroke_2,Tween,{
				Transparency = 0
			});

			Fallens:_Animation(UIStroke,Tween,{
				Transparency = 0
			});

			Fallens:_Animation(ColorPickBox,Tween,{
				BackgroundTransparency = 0,
				ImageTransparency = 0
			});

			Fallens:_Animation(MouseMovement,Tween,{
				ImageTransparency = 0
			});

			Fallens:_Animation(ColorOpc,Tween,{
				BackgroundTransparency = 0
			});

			Fallens:_Animation(TransparentImage,Tween,{
				ImageTransparency = 0
			});

			Fallens:_Animation(Left,Tween,{
				BackgroundTransparency = 0
			});

			Fallens:_Animation(Left_2,Tween,{
				BackgroundTransparency = 0
			});

			Fallens:_Animation(Right,Tween,{
				BackgroundTransparency = 0
			});

			Fallens:_Animation(Right_2,Tween,{
				BackgroundTransparency = 0
			});

			Fallens:_Animation(ColorRedGreenBlue,Tween,{
				BackgroundTransparency = 0
			});

			Fallens:_Animation(HexFrame,Tween,{
				BackgroundTransparency = 0
			});

			Fallens:_Animation(TextLabel,Tween,{
				TextTransparency = 0
			});
		else
			Fallens:_Animation(UIStroke_8,Tween,{
	...
	function Args:Update()
		...
		Fallens:_Animation(ColorPickBox,TweenInfo.new(0.2),{
			BackgroundColor3 = Color3.fromHSV(H , 1 , 1)
		});

		Fallens:_Animation(ColorOpc,TweenInfo.new(0.2),{
			BackgroundColor3 = RealColor
		});

		Fallens:_Animation(MouseMovement,TweenInfo.new(0.2),{
			Position = UDim2.fromScale(S , 1 - V)
		});

		Fallens:_Animation(ColorOptSlide,TweenInfo.new(0.2),{
			Position = UDim2.new(Transparency ,0 , 0.5 ,0)
		});

		Fallens:_Animation(ColorRGBSlide,TweenInfo.new(0.2),{
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Fallens:_IsMouseOverFrame(ColorPickerWindow) then
				ToggleUI(false);
			end;
		end;
	end)
...
function Fallens:_DrawKeybinds(Window: ScreenGui)
	if Fallens.__KEYBINDS_CACHE then
		return Fallens.__KEYBINDS_CACHE;
	end;
	...
	Keybinds.Name = Fallens:_RandomString()
	Keybinds.Parent = Window
	Keybinds.BackgroundColor3 = Fallens.Colors.BGDBColor;
	...
	IconFrame.Name = Fallens:_RandomString()
	IconFrame.Parent = Keybinds
	IconFrame.AnchorPoint = Vector2.new(1, 0.5)
	IconFrame.BackgroundColor3 = Fallens.Colors.BGDBColor
	...
	Icon.Name = Fallens:_RandomString()
	...
	Icon.ZIndex = 159
	Icon.Image = Fallens:CacheImage("rbxassetid://10723416765");

	HeaderFrame.Name = Fallens:_RandomString()
	...
	HeadLabel.Name = Fallens:_RandomString()
	...
	HeadLabel.TextColor3 = Fallens.Colors.SwitchColor
	...
	MainFrame.Name = Fallens:_RandomString()
	...
	MovingFrame.Name = Fallens:_RandomString()
	...
	Fallens:Drag(MovingFrame,Keybinds,0.1);
	...
	Ref.THREAD = task.spawn(function()
		while true do task.wait()
			Fallens:_Animation(MainFrame,TweenInfo.new(0.4),{
				Size = UDim2.new(1, 30, 1, UIListLayout.AbsoluteContentSize.Y + 1)
			});

			if UIListLayout.AbsoluteContentSize.Y > 1 then
				Fallens:_Animation(IconFrame,TweenInfo.new(0.25),{
					BackgroundTransparency = 0.3
				})

				Fallens:_Animation(Frame,TweenInfo.new(0.25),{
					BackgroundTransparency = 0
				})

				Fallens:_Animation(HeadLabel,TweenInfo.new(0.25),{
					TextTransparency = 0
				})

				Fallens:_Animation(Icon,TweenInfo.new(0.25),{
					ImageTransparency = 0
				});
				...
				Fallens:_Animation(Keybinds,TweenInfo.new(0.25),{
			...
			if Fallens.PerformanceMode then
				if Keybinds.Visible then
					Fallens:_SetNilP(Keybinds , Window);
				else
					Fallens:_SetNilP(Keybinds , nil);
				end;
			else
				Fallens:_SetNilP(Keybinds , Window);
			end;
	...
	function Ref:AddFrame()
		...
		Keyholder.Name = Fallens:_RandomString()
		Keyholder.BackgroundColor3 = Fallens.Colors.BGDBColor
		...
		Label.Name = Fallens:_RandomString()
		...
		Label.TextColor3 = Fallens.Colors.SwitchColor
		...
		Line.Name = Fallens:_RandomString()
		Line.Parent = Keyholder
		Line.AnchorPoint = Vector2.new(1, 0.5)
		Line.BackgroundColor3 = Fallens.Colors.BGDBColor
		...
		TypeLabel.Name = Fallens:_RandomString()
		...
		TypeLabel.TextColor3 = Fallens.Colors.SwitchColor
		...
		function frame_ref:SetVisible(v)
			if v then
				Fallens:_Animation(Keyholder,TweenInfo.new(0.1),{
					BackgroundTransparency = 0.600,
					Size = UDim2.new(1, 0, 0, 28)
				});

				Fallens:_Animation(Label,TweenInfo.new(0.15),{
					TextTransparency = 0.100
				});

				Fallens:_Animation(Line,TweenInfo.new(0.15),{
					BackgroundTransparency = 0
				});

				Fallens:_Animation(TypeLabel,TweenInfo.new(0.15),{
					TextTransparency = 0
				});
			else
				Fallens:_Animation(Keyholder,TweenInfo.new(0.1),{
	...
	Fallens.__KEYBINDS_CACHE = Ref;

	return Ref;
end;

function Fallens:_KeybindHandler(Parent: Frame , ObjectType: string , ElementAPI: Toggle & Slider , Signal , Zindex: number , ElementCFG: Slider)
	local Window = Fallens:_GetWindowFromElement(Parent);
	local KB_Signal = Fallens.__SIGNAL(false);
	local SubIndex = math.random(40,100);
	local KeybindInd = Fallens:_DrawKeybinds(Window);
	...
	KeybindHandler.Name = Fallens:_RandomString()
	KeybindHandler.Parent = Window
	KeybindHandler.BackgroundColor3 = Fallens.Colors.BlockBackground
	...
	ElementObjs.Name = Fallens:_RandomString()
	...
	local refreshPF = function()
		if Fallens.PerformanceMode then
			if KeybindHandler.Size.Y.Offset > 1 then
				Fallens:_SetNilP(KeybindHandler , Window);
			else
				Fallens:_SetNilP(KeybindHandler , nil);
			end;
		else
			Fallens:_SetNilP(KeybindHandler , Window);
		end;
	...
	local ToggleUI = function(bool)
		if bool then
			...
			Fallens:_Animation(KeybindHandler,TweenInfo.new(0.25),{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 225, 0, UIListLayout.AbsoluteContentSize.Y + 5)
			});

			Fallens:_Animation(UIStroke,TweenInfo.new(0.3),{
				Transparency = 0
			});
		else
			Fallens:_Animation(KeybindHandler,TweenInfo.new(0.3),{
				BackgroundTransparency = 1,
	...
	local ElementAPIs = Fallens:_LoadElement(ElementObjs , true , KB_Signal , true);
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.MouseButton2 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Fallens:_IsMouseOverFrame(Parent) and not Fallens:_IsMouseOverFrame(KeybindHandler) then
...
function Fallens:_LoadOption(Value , TabSignal)
	...
	function Args:AddKeybind(Config: MiniKeybind)
		...
		Fallens:_Input(Keybind.Root,function()
			if IsBinding then
				return;
			end;
		...
		if Config.Flag then
			Fallens.Flags[Config.Flag] = Args;
		end;
	...
	function Args:AddHelper(Config: Helper)
		...
		Helper.UIStroke:GetPropertyChangedSignal('Transparency'):Connect(function()
			...
			if Fallens.PerformanceMode then
				if Helper.Text.Visible then
					Fallens:_SetNilP(Helper.Text , Window);
				else
					Fallens:_SetNilP(Helper.Text , nil);
				end;
			else
				Fallens:_SetNilP(Helper.Text , Window);
			end;
		...
		local Update = function()
			...
			Fallens:_Animation(Helper.Text , TweenInfo.new(0.15), {
		...
		local Release = function()
			...
			Fallens:_Animation(Helper.UIStroke,TweenInfo.new(0.15),{
				Transparency = 1
			});
		end;
		...
		Fallens:_Hover(Button,  Hold, Release);
	...
	function Args:AddColorPicker(Config: MiniColorPicker)
		...
		local Button = Fallens:_Input(ColorPicker);

		local ColorPicker = Fallens:_AddColorPickerPanel(Button,function(color,opc)
	...
	function Args:AddOption()
		...
		local Signal = Fallens.__SIGNAL(false);
		...
		local ToggleUI = function(bool)
			...
			if bool then
				...
				Fallens:_Animation(ExtractElement , Tween , {
					Position = MainPosition,
					BackgroundTransparency = 0,
		...
		Fallens:_AddPropertyEvent(ExtractElement,function(bool)
			ExtractElement.Visible = bool;

			if Fallens.PerformanceMode then
				if ExtractElement.Visible then
					Fallens:_SetNilP(ExtractElement , Window);
				else
					Fallens:_SetNilP(ExtractElement , nil);
				end;
			else
				Fallens:_SetNilP(ExtractElement , Window);
			end;
		...
		Fallens:_AddDragBlacklist(ExtractElement);

		ExtractElement.Name = Fallens:_RandomString()
		ExtractElement.Parent = Window
		ExtractElement.BackgroundColor3 = Fallens.Colors.BlockBackground
		...
		Elements.Name = Fallens:_RandomString()
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if Toggl and not Fallens:_IsMouseOverFrame(ExtractElement) and not Fallens:_IsMouseOverFrame(Element) then
					ToggleUI(false);
				end;
			end
		end)		

		return Fallens:_LoadElement(Elements , true , Signal)
	end;

	return Args;
end;

function Fallens:_LoadDropdown(BaseParent: TextButton , Callback: () -> any)
	local Window = Fallens:_GetWindowFromElement(BaseParent);
	...
	local UIListLayout = Instance.new("UIListLayout")
	local ToggleDb = Fallens.__SIGNAL(false);
	local EventOut = Fallens.__SIGNAL(0);

	DropdownWindow.Name = Fallens:_RandomString()
	DropdownWindow.Parent = Window
	DropdownWindow.BackgroundColor3 = Fallens.Colors.BlockBackground
	...
	Fallens:_AddDragBlacklist(DropdownWindow);
	Fallens:_AddPropertyEvent(DropdownWindow,function(v)
		DropdownWindow.Visible = v;

		if Fallens.PerformanceMode then
			if DropdownWindow.Visible then
				Fallens:_SetNilP(DropdownWindow , Window);
			else
				Fallens:_SetNilP(DropdownWindow , nil);
			end;
		else
			Fallens:_SetNilP(DropdownWindow , Window);
		end;
	...
	local DrawButton = function()
		...
		DropdownItem.Name = Fallens:_RandomString()
	...
	local UpdateDropdown = function()
		...
		for i,v in next , Values do
			...
			table.insert(__signals , ToggleDb:Connect(function(bool)
				if bool then
					Fallens:_Animation(bth.BlockText,TweenInfo.new(0.2),{
						TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
					});

					Fallens:_Animation(bth.BlockLine,TweenInfo.new(0.2),{
						BackgroundTransparency = 0
					});
				else
					Fallens:_Animation(bth.BlockText,TweenInfo.new(0.2),{
			...
			if IsMulti then
				...
				Fallens:_Input(bth.DropdownItem,function()
				...
				Fallens:_Input(bth.DropdownItem,function()
					if DataFrame then
						Fallens:_Animation(DataFrame.BlockText,TweenInfo.new(0.2),{
							TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
						});
					end;
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Fallens:_IsMouseOverFrame(DropdownWindow) then
				ToggleUI(false);
			end;
		end;
	end);
...
function Fallens:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal , DisableStackKeybind)
	...
	function Args:AddToggle(Config : Toggle)
		...
		local Block = Fallens:_CreateBlock(Signal);
		...
		if Config.Risky then
			Block:SetTextColor(Fallens.Colors.Risky);
		end;
		...
		Args.Link = Fallens:_LoadOption(Block);
		...
		if not DisableStackKeybind then
			local AutoKeybind = Fallens:_KeybindHandler(Block.Root , "Toggle" , Args , Signal , Zindex , Config);
	...
	function Args:AddButton(Config: Button)
		...
		if Fallens:_IsMobile() then
			Fallens:_AddDragBlacklist(Button);
		end;

		Button.Name = Fallens:_RandomString()
		...
		Fallens:_Hover(Frame,function()
			if Signal:GetValue() then
				Fallens:_Animation(Frame,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				})
			end;
		end,function()
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				Fallens:_Animation(BlockLine, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.500
				});

				Fallens:_Animation(Frame, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.1
				});

				Fallens:_Animation(UIStroke, TweenInfo.new(0.35),{
					Transparency = 0
				});

				Fallens:_Animation(TextLabel, TweenInfo.new(0.35),{
	...
	function Args:AddSlider(Config: Slider)
		...
		Fallens:_AddDragBlacklist(Slider);

		Slider.Name = Fallens:_RandomString()
		...
		SliderBar.Name = Fallens:_RandomString()
		SliderBar.Parent = Slider
		SliderBar.AnchorPoint = Vector2.new(0.5, 1)
		SliderBar.BackgroundColor3 = Fallens.Colors.DropColor
		...
		SliderInput.Name = Fallens:_RandomString()
		SliderInput.Parent = SliderBar
		SliderInput.AnchorPoint = Vector2.new(0, 0.5)
		SliderInput.BackgroundColor3 = Fallens.Colors.Highlight
		...
		Frame.BackgroundColor3 = Fallens.Colors.SwitchColor
		Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Frame.BorderSizePixel = 0
		Frame.Position = UDim2.new(1, 5, 0.5, 0)
		...
		ValueText.Name = Fallens:_RandomString()
		...
		ValueText.TextColor3 = Fallens.Colors.SwitchColor
		...
		Fallens:_Hover(SliderBar,function()
			if Signal:GetValue() then
				Fallens:_Animation(ValueText,TweenInfo.new(0.2),{
		...
		local Update = function(Input)
			...
			local Value = Fallens:_Rounding(Main,Config.Round);
			...
			SliderBar.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not Fallens:_IsMouseOverFrame(SliderBar) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		function Args:SetValue(Value)
			...
			Fallens:_Animation(SliderInput, TweenInfo.new(0.35),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Fallens:_Animation(ValueText,Tween,{
					TextTransparency = 0.750
				})
				...
				Fallens:_Animation(SliderInput,Tween,{
					BackgroundTransparency = 0
				})
				...
				Fallens:_Animation(SliderBar,Tween,{
					BackgroundTransparency = 0
				})

				Fallens:_Animation(BlockLine,Tween,{
					BackgroundTransparency = 0.5
				})

				Fallens:_Animation(BlockText,Tween,{
		...
		if not DisableStackKeybind then
			local AutoKeybind = Fallens:_KeybindHandler(Slider , "Number" , Args , Signal , Zindex , Config);
	...
	function Args:AddParagraph(Config: Paragraph) -- request by Neptune
		...
		if Fallens:_IsMobile() then
			Fallens:_AddDragBlacklist(Paragraph);
		end;

		Paragraph.Name = Fallens:_RandomString()
		...
		DescriptionText.Name = Fallens:_RandomString()
		...
		DescriptionText.TextColor3 = Fallens.Colors.SwitchColor
		...
		local UpdateScale = function()
			...
			if not DescriptionText.Text:byte() then
				...
				Fallens:_Animation(Paragraph,TweenInfo.new(0.15),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				Fallens:_Animation(BlockText,TweenInfo.new(0.2),{
					TextTransparency = 0.300
				});

				Fallens:_Animation(DescriptionText,TweenInfo.new(0.2),{
					TextTransparency = 0.500
				});

				Fallens:_Animation(BlockLine,TweenInfo.new(0.2),{
					BackgroundTransparency = 0.500
				});
			else
				Fallens:_Animation(BlockText,TweenInfo.new(0.2),{
	...
	function Args:AddTextBox(Config: TextBoxConfig)
		...
		if Fallens:_IsMobile() then
			Fallens:_AddDragBlacklist(TextBox);
		end;

		TextBox.Name = Fallens:_RandomString()
		...
		LinkValues.AnchorPoint = Vector2.new(1, 0.540000021)
		LinkValues.BackgroundColor3 = Fallens.Colors.DropColor
		LinkValues.BorderColor3 = Color3.fromRGB(0, 0, 0)
		LinkValues.BorderSizePixel = 0
		LinkValues.Position = UDim2.new(1, -12, 0.5, 0)
		...
		TextBox_2.TextColor3 = Fallens.Colors.SwitchColor
		...
		local Update = function()
			...
			local xp = pcall(function()
				Fallens:_Animation(LinkValues,TweenInfo.new(0.25),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Fallens:_Animation(LinkValues,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
	...
	function Args:AddDropdown(Config : Dropdown)
		...
		Dropdown.Name = Fallens:_RandomString()
		...
		ValueItems.Name = Fallens:_RandomString()
		ValueItems.Parent = Dropdown
		ValueItems.AnchorPoint = Vector2.new(0.5, 1)
		ValueItems.BackgroundColor3 = Fallens.Colors.DropColor
		...
		MainButton.Name = Fallens:_RandomString()
		...
		MainButton.Image = Fallens:CacheImage("rbxassetid://109535175596957")

		Fallens:_Hover(ValueItems,function()
			Fallens:_Animation(ValueItems,TweenInfo.new(0.3),{
		...
		local repi;
		local Button = Fallens:_Input(ValueItems);

		repi = Fallens:_LoadDropdown(Button,function(value)
			Config.Default = value;
		...
		repi.EventOut:Connect(function(v)
			if v then
				Fallens:_Animation(MainButton,TweenInfo.new(0.2),{
					Rotation = -180
				})
			else
				Fallens:_Animation(MainButton,TweenInfo.new(0.2),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Fallens:_Animation(ValueItems,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
				...
				Fallens:_Animation(ValueText,TweenInfo.new(0.32),{
					TextTransparency = 0
				});
		...
		Args.Link = Fallens:_LoadOption({
			AddLink = function(self ,Name , Default)
				return Fallens:_AddLinkValue(Name , Default , LinkValues , LinkValues , {
...
function Fallens:GetTheme()
	return Fallens.Colors;
end;

function Fallens:SetTheme(name)
	...
	Fallens:RefreshCurrentColor()
end;

function Fallens:RefreshCurrentColor()
	for i,v in next , Fallens.Elements.Highlight do
		if v.Element and v.Property then
			v.Element[v.Property] = Fallens.Colors.Highlight;
		end;
	end;

	for i,v in next , Fallens.Elements do
		if v.Element and v.Property and v.Element:GetAttribute('Enabled') then
			v.Element[v.Property] = Fallens.Colors.Highlight;
		end;
	end;

	for i,v in next , Fallens.Elements.Risky do
		if v.Element and v.Property then
			v.Element[v.Property] = Fallens.Colors.Risky;
		end;
	end;

	for i,v in next , Fallens.Elements.BlockColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Fallens.Colors.BlockColor;
		end;
	end;

	for i,v in next , Fallens.Elements.BGDBColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Fallens.Colors.BGDBColor;
		end;
	end;

	for i,v in next , Fallens.Elements.StrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Fallens.Colors.StrokeColor;
		end;
	end;

	for i,v in next , Fallens.Elements.SwitchColor do
		if v.Element and v.Property and v.Element[v.Property] ~= Fallens.Colors.MouseEnter then
			v.Element[v.Property] = Fallens.Colors.SwitchColor;
		end;
	end;

	for i,v in next , Fallens.Elements.BlockBackground do
		if v.Element and v.Property and v.Element[v.Property] then
			v.Element[v.Property] = Fallens.Colors.BlockBackground;
		end;
	end;

	for i,v in next , Fallens.Elements.DropColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Fallens.Colors.DropColor;
		end;
	end;

	for i,v in next , Fallens.Elements.LineColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Fallens.Colors.LineColor;
		end;
	end;

	for i,v in next , Fallens.Elements.HighStrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Fallens.Colors.HighStrokeColor;
		end;
	end;
end;

function Fallens:ChangeHighlightColor(NewColor: Color3)
	local H,S,V = NewColor:ToHSV();

	Fallens.Colors.Highlight = NewColor;
	Fallens.Colors.Toggle = Color3.fromHSV(H,S,V - 0.2);
...
function Fallens.new(Config : Window)

	if not Config.Scale then
		if Fallens:_IsMobile() then
			Config.Scale = Fallens.Scale.Mobile;
		else
			Config.Scale = Fallens.Scale.Window;
		end;
	end;
	...
	Config = Fallens.__CONFIG(Config , {
		...
		Keybind = "Insert",
		Logo = Fallens.Logo;
		Scale = Fallens.Scale.Window,
		TextSize = 15
	});

	local TabHover = Fallens.__SIGNAL(false);
	local WindowOpen = Fallens.__SIGNAL(true);
	local WindowArgs = {
		SelectedTab = nil,
		Tabs = {},
		LastTab = nil,
	...
	local FallensGui = Instance.new("ScreenGui")
	local MainFrame = Instance.new("Frame")
	local UICorner = Instance.new("UICorner")
	...
	Fallens:_DrawKeybinds(FallensGui);

	UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
		TabButtonScrollingFrame.CanvasSize = UDim2.fromOffset(0,UIListLayout.AbsoluteContentSize.Y)
	end);

	FallensGui.Name = "u?name=Fallens_?"..Fallens:_RandomString();
	FallensGui.Parent = CoreGui;
	FallensGui.ResetOnSpawn = false
	FallensGui.IgnoreGuiInset = true;
	FallensGui.ZIndexBehavior = Enum.ZIndexBehavior.Global;

	Fallens.ProtectGui(FallensGui);

	WindowArgs.Root = FallensGui;

	table.insert(Fallens.Windows , FallensGui);

	MainFrame.Active = true;
	MainFrame.Name = Fallens:_RandomString()
	MainFrame.Parent = FallensGui
	MainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
	MainFrame.BackgroundColor3 = Fallens.Colors.BGDBColor
	...
	MainFrame.Size = Fallens.Scale.Window
	MainFrame.ZIndex = 4
	...
	Fallens:_Animation(MainFrame,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
		Size = Config.Scale
	});
	...
	TabFrame.Name = Fallens:_RandomString()
	TabFrame.Parent = MainFrame
	TabFrame.AnchorPoint = Vector2.new(1, 0)
	TabFrame.BackgroundColor3 = Fallens.Colors.BGDBColor
	...
	LineFrame1.Name = Fallens:_RandomString()
	LineFrame1.Parent = TabFrame
	LineFrame1.AnchorPoint = Vector2.new(1, 0)
	LineFrame1.BackgroundColor3 = Fallens.Colors.BGDBColor
	...
	LogoBox.Name = Fallens:_RandomString()
	LogoBox.Parent = TabFrame
	LogoBox.BackgroundColor3 = Fallens.Colors.Highlight
	LogoBox.BackgroundTransparency = 0.860
	LogoBox.BorderColor3 = Color3.fromRGB(0, 0, 0)
	LogoBox.BorderSizePixel = 0
	LogoBox.Position = UDim2.new(0, 9, 0, 7)
	...
	LogoBoxCorner.CornerRadius = UDim.new(0, 12)
	LogoBoxCorner.Parent = LogoBox

	LogoBoxStroke.Color = Fallens.Colors.Highlight
	LogoBoxStroke.Transparency = 0.650
	LogoBoxStroke.Thickness = 1
	LogoBoxStroke.Parent = LogoBox

	table.insert(Fallens.Elements.Highlight , {
		Element = LogoBoxStroke,
		Property = 'Color'
	});

	CompLogo.Name = Fallens:_RandomString()
	CompLogo.Parent = LogoBox
	...
	CompLogo.Position = UDim2.new(0.5, 0, 0.5, 0)
	CompLogo.Size = UDim2.new(0, 24, 0, 24)
	CompLogo.Image = Config.Logo
	
	if Fallens.CustomHighlightMode then
		CompLogo.ImageColor3 = Fallens.Colors.Highlight;
	...
	WindowLabel.Name = Fallens:_RandomString()
	...
	WindowLabel.Size = UDim2.new(0, 200, 0, 22)
	WindowLabel.Font = Enum.Font.GothamBold
	WindowLabel.Text = Config.Name
	WindowLabel.TextColor3 = Fallens.Colors.SwitchColor
	...
	SubLabel.Name = Fallens:_RandomString()
	SubLabel.Parent = TabFrame
	SubLabel.BackgroundTransparency = 1.000
	SubLabel.BorderColor3 = Color3.fromRGB(0, 0, 0)
	SubLabel.BorderSizePixel = 0
	SubLabel.Position = UDim2.new(0, 60, 0, 32)
	SubLabel.Size = UDim2.new(0, 200, 0, 16)
	SubLabel.Font = Enum.Font.Gotham
	SubLabel.Text = Config.SubName or "Script Hub"
	SubLabel.TextColor3 = Fallens.Colors.SwitchColor
	SubLabel.TextTransparency = 0.550
	SubLabel.TextSize = 11.000
	...
	HeaderDivider.Name = Fallens:_RandomString()
	HeaderDivider.Parent = TabFrame
	HeaderDivider.BackgroundColor3 = Fallens.Colors.SwitchColor
	HeaderDivider.BackgroundTransparency = 0.930
	HeaderDivider.BorderColor3 = Color3.fromRGB(0, 0, 0)
	HeaderDivider.BorderSizePixel = 0
	HeaderDivider.Position = UDim2.new(0, 12, 0, 60)
	HeaderDivider.Size = UDim2.new(1, -37, 0, 1)

	table.insert(Fallens.Elements.SwitchColor , {
		Element = HeaderDivider,
		Property = 'BackgroundColor3'
	});
	...
	SearchBar.Name = Fallens:_RandomString()
	SearchBar.Parent = TabFrame
	SearchBar.BackgroundColor3 = Fallens.Colors.BlockColor
	SearchBar.BorderColor3 = Color3.fromRGB(0, 0, 0)
	SearchBar.BorderSizePixel = 0
	SearchBar.Position = UDim2.new(0, 12, 0, 70)
	SearchBar.Size = UDim2.new(1, -37, 0, 32)
	...
	SearchIcon.Name = Fallens:_RandomString()
	...
	SearchIcon.Image = Fallens:_GetIcon("search")
	SearchIcon.ImageColor3 = Fallens.Colors.SwitchColor
	...
	SearchInput.Name = Fallens:_RandomString()
	...
	SearchInput.Position = UDim2.new(0, 31, 0.5, 0)
	SearchInput.Size = UDim2.new(1, -75, 1, 0)
	SearchInput.Font = Enum.Font.Gotham
	SearchInput.PlaceholderText = "Search.."
	SearchInput.Text = ""
	SearchInput.TextColor3 = Fallens.Colors.SwitchColor
	...
	KeyBadge.Name = Fallens:_RandomString()
	KeyBadge.Parent = SearchBar
	KeyBadge.AnchorPoint = Vector2.new(1, 0.5)
	KeyBadge.BackgroundColor3 = Fallens.Colors.HighStrokeColor
	KeyBadge.BorderColor3 = Color3.fromRGB(0, 0, 0)
	...
	KeyBadgeText.Name = Fallens:_RandomString()
	KeyBadgeText.Parent = KeyBadge
	KeyBadgeText.AnchorPoint = Vector2.new(0.5, 0.5)
	KeyBadgeText.BackgroundTransparency = 1.000
	KeyBadgeText.BorderColor3 = Color3.fromRGB(0, 0, 0)
	KeyBadgeText.BorderSizePixel = 0
	KeyBadgeText.Position = UDim2.new(0.5, 0, 0.5, 0)
	KeyBadgeText.Size = UDim2.new(1, 0, 1, 0)
	KeyBadgeText.Font = Enum.Font.GothamMedium
	KeyBadgeText.Text = "⌘K"
	KeyBadgeText.TextColor3 = Fallens.Colors.SwitchColor
	KeyBadgeText.TextTransparency = 0.400
	KeyBadgeText.TextSize = 11.000

	table.insert(Fallens.Elements.SwitchColor , {
		Element = KeyBadgeText,
		Property = 'TextColor3'
	});
	...
	SearchInput:GetPropertyChangedSignal('Text'):Connect(function()
		...
		for _, Node in next , TabButtonScrollingFrame:GetChildren() do
			if Node:IsA("Frame") then
				local Tag = Node:GetAttribute("SearchTag");

				if Tag then
					Node.Visible = (Query == "") or string.find(string.lower(Tag), Query, 1, true) ~= nil;
				end;
			end;
		end;
	end);

	TabButtons.Name = Fallens:_RandomString()
	TabButtons.Parent = TabFrame
	TabButtons.BackgroundTransparency = 1.000
	TabButtons.BorderColor3 = Color3.fromRGB(0, 0, 0)
	TabButtons.BorderSizePixel = 0
	TabButtons.Position = UDim2.new(0, 0, 0, 116)
	TabButtons.Size = UDim2.new(1, -25, 1, -181)

	SelectionFrame.Name = Fallens:_RandomString()
	SelectionFrame.Parent = TabButtons
	SelectionFrame.AnchorPoint = Vector2.new(1, 0)
	SelectionFrame.BackgroundColor3 = Fallens.Colors.Highlight
	...
	TabButtonScrollingFrame.Name = Fallens:_RandomString()
	...
	Userinfo.Name = Fallens:_RandomString()
	...
	do
		...
		HighlightStroke.Color = Fallens.Colors.Highlight
		HighlightStroke.Transparency = 0.850
		...
		Fallens:_Input(Userinfo,function()
	...
	UserProfile.Name = Fallens:_RandomString()
	...
	UserProfile.Image = Fallens:CacheImage("rbxassetid://18518299306")

	UICorner_4.CornerRadius = UDim.new(1, 0)
	UICorner_4.Parent = UserProfile

	UserText.Name = Fallens:_RandomString()
	...
	UserText.TextColor3 = Fallens.Colors.SwitchColor
	...
	ExpireText.Name = Fallens:_RandomString()
	...
	ExpireText.TextColor3 = Fallens.Colors.SwitchColor
	...
	TabMainFrame.Name = Fallens:_RandomString()
	...
	if Fallens:_IsMobile() then
		Fallens:_AddDragBlacklist(TabButtons);
	end;
	...
	WindowOpen:Connect(function(v)
		...
		if v then
			Fallens:_Animation(MainFrame,TweenInfo.new(0.2),{
				Size = Config.Scale
			})

			Fallens:_Animation(TabButtonScrollingFrame,TweenInfo.new(0.35),{
				Position = UDim2.new(0.5, 0, 0.5, 0)
			})

			Fallens:_Animation(CompLogo,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			Fallens:_Animation(WindowLabel,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			Fallens:_Animation(SubLabel,TweenInfo.new(0.2),{
				TextTransparency = 0.550
			})

			Fallens:_Animation(SearchBar,TweenInfo.new(0.2),{
				BackgroundTransparency = 0
			})

			Fallens:_Animation(SearchIcon,TweenInfo.new(0.2),{
				ImageTransparency = 0.5
			})

			Fallens:_Animation(SearchInput,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			Fallens:_Animation(KeyBadge,TweenInfo.new(0.2),{
				BackgroundTransparency = 0
			})

			Fallens:_Animation(KeyBadgeText,TweenInfo.new(0.2),{
				TextTransparency = 0.4
			})

			Fallens:_Animation(UserProfile,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			Fallens:_Animation(UserText,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			Fallens:_Animation(ExpireText,TweenInfo.new(0.2),{
			...
			Fallens:_Animation(LineFrame1,TweenInfo.new(0.3),{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 20, 1, 0)
			})

			Fallens:_Animation(TabFrame,TweenInfo.new(0.2),{
			...
			Fallens:_Animation(LineFrame1,TweenInfo.new(0.1),{
				BackgroundTransparency = 1,
			...
			Fallens:_Animation(TabFrame,TweenInfo.new(0.1),{
				BackgroundTransparency = 1
			})
		end;
	end);
	...
	TabHover:Connect(function(value)
		...
		if value then
			Fallens:_Animation(TabFrame , Style , {
				Size = UDim2.new(0, 185,1, 0)
			});

			Fallens:_Animation(WindowLabel , Style , {
				Position = UDim2.new(0, 60,0, 12),
				TextTransparency = 0
			});

			Fallens:_Animation(SubLabel , Style , {
				Position = UDim2.new(0, 60,0, 32),
				TextTransparency = 0.550
			});

			Fallens:_Animation(HeaderDivider , Style , {
				BackgroundTransparency = 0.930
			});

			Fallens:_Animation(SearchBar , Style , {
				BackgroundTransparency = 0
			});

			Fallens:_Animation(SearchIcon , Style , {
				ImageTransparency = 0.5
			});

			Fallens:_Animation(SearchInput , Style , {
				TextTransparency = 0
			});

			Fallens:_Animation(KeyBadge , Style , {
				BackgroundTransparency = 0
			});

			Fallens:_Animation(KeyBadgeText , Style , {
				TextTransparency = 0.4
			});

			Fallens:_Animation(UserText , Style , {
				Position = UDim2.new(0, 55,0, 8),
				TextTransparency = 0.1
			});

			Fallens:_Animation(ExpireText , Style , {
			...
			Fallens:_Animation(WindowLabel , Style , {
				Position = UDim2.new(0, 60 + 25,0, 12),
				TextTransparency = 1
			});

			Fallens:_Animation(SubLabel , Style , {
				Position = UDim2.new(0, 60 + 25,0, 32),
				TextTransparency = 1
			});
	...
	do
		...
		UserSettings.Name = Fallens:_RandomString()
		UserSettings.Parent = FallensGui;
		UserSettings.BackgroundColor3 = Fallens.Colors.BGDBColor;
		...
		SectionFrame.Name = Fallens:_RandomString()
		SectionFrame.Parent = UserSettings
		SectionFrame.BackgroundColor3 = Fallens.Colors.BGDBColor;
		...
		ImageLabel.Image = Fallens:CacheImage("rbxassetid://10747384394")
		...
		function WindowArgs.UserSettings:Create()
			...
			WindowArgs.UserSettings.Signal = Fallens:_Blur(UserSettings,Signal);

			Fallens:Drag(UserSettings , UserSettings, 0.15);

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				Fallens:_Animation(UserSettings,TweenInfo.new(0.2),{
			...
			function WindowArgs.UserSettings:Window(Value)
				if Value then
					...
					Fallens:_Animation(SectionFrame,TweenInfo.new(0.2),{
						BackgroundTransparency = 0,
					});

					Fallens:_Animation(HeaderText,TweenInfo.new(0.2),{
						TextTransparency = 0,
					});

					Fallens:_Animation(ImageLabel,TweenInfo.new(0.2),{
						ImageTransparency = 0.5,
					});
				else
					Signal:Fire(false);
			...
			Fallens:_Input(ImageLabel,function()
				WindowArgs.UserSettings:Window(false);
			end);

			WindowArgs.UserSettings:Window(false);

			return Fallens:_LoadElement(SectionFrame , true , Signal);
		end;
	end;

	function WindowArgs:SetVisible(bool: boolean)
		FallensGui.Enabled = bool;
	end;
	...
	function WindowArgs:DrawCategory(config : Category)
		...
		Category.Size = UDim2.new(1, -10, 0, 24)
		Category:SetAttribute("SearchTag", config.Name)

		if Fallens:_IsMobile() then
			Fallens:_AddDragBlacklist(Category);
		end;

		CategoryText.Name = Fallens:_RandomString()
		...
		CategoryText.Position = UDim2.new(0, 8, 0, 10)
		CategoryText.Size = UDim2.new(1, 200, 0, 12)
		CategoryText.Font = Enum.Font.GothamMedium
		CategoryText.Text = string.upper(config.Name)
		CategoryText.TextColor3 = Fallens.Colors.SwitchColor
		CategoryText.TextSize = 11.000
		CategoryText.TextTransparency = 0.600
	...
	function WindowArgs:DrawContainerTab(TabConfig : ContainerTab)
		TabConfig = Fallens.__CONFIG(TabConfig,{
			Name = "Tab",
			Icon = "eye",
		});

		local Tween = TweenInfo.new(0.35,Enum.EasingStyle.Quint);
		local TabOpenSignal = Fallens.__SIGNAL(false);
		...
		TabButton.Name = Fallens:_RandomString()
		...
		TabButton.ZIndex = 3
		TabButton:SetAttribute("SearchTag", TabConfig.Name)

		if Fallens:_IsMobile() then
			Fallens:_AddDragBlacklist(TabButton);
		end;

		Icon.Name = Fallens:_RandomString()
		Icon.Parent = TabButton
		Icon.AnchorPoint = Vector2.new(0, 0.5)
		Icon.BackgroundColor3 = Fallens.Colors.Highlight
		Icon.BackgroundTransparency = 1.000
		Icon.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Icon.BorderSizePixel = 0
		...
		Icon.ZIndex = 3
		Icon.Image = Fallens:_GetIcon(TabConfig.Icon);
		Icon.ImageColor3 = Fallens.Colors.Highlight
		...
		TabNameLabel.Name = Fallens:_RandomString()
		...
		TabNameLabel.TextColor3 = Fallens.Colors.SwitchColor
		...
		Highlight.BackgroundColor3 = Color3.fromRGB(161, 161, 161)
		Highlight.BackgroundTransparency = 0.870
		Highlight.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Highlight.BorderSizePixel = 0
		Highlight.Position = UDim2.new(0.5, 0, 0.5, 0)
		Highlight.Size = UDim2.new(1, -14, 1, -6)
		Highlight.ZIndex = 2
		...
		ContainerTab.Name = Fallens:_RandomString()
		...
		Top.Name = Fallens:_RandomString()
		...
		Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
			...
			if Fallens.PerformanceMode then
				if ContainerTab.Visible then
					Fallens:_SetNilP(ContainerTab , TabMainFrame);
				else
					Fallens:_SetNilP(ContainerTab , nil);
				end;
			else
				Fallens:_SetNilP(ContainerTab , TabMainFrame);
			end;
		...
		local TabOpen = function(bool)
			if bool then
				...
				Fallens:_Animation(Icon,Tween,{
					ImageTransparency = 0,
				});

				Fallens:_Animation(TabNameLabel,Tween,{
					TextTransparency = 0
				});

				Fallens:_Animation(Highlight,Tween,{
					BackgroundTransparency = 0.925
				});
		...
		Fallens:_Hover(TabButton,function()
		...
		TabHover:Connect(function(bool)
			if bool then
				Fallens:_Animation(TabButton,Tween,{
				...
				Fallens:_Animation(UICorner,Tween,{
		...
		Fallens:_Input(TabButton,function()
		...
		function TabArgs:DrawTab(TabConfig : TabConfig) -- Internal Tab
			...
			local InternalSignal = Fallens.__SIGNAL(false);
			local Frame = Instance.new("Frame")
			local UICorner = Instance.new("UICorner")
			local UIStroke = Instance.new("UIStroke")
			local Highlight = Instance.new("Frame")
			...
			Frame.Parent = Top
			Frame.BackgroundColor3 = Fallens.Colors.BlockColor
			...
			local ToggleUI = function(bool)
				...
				if bool then
					...
					Fallens:_Animation(TextLabel,TweenInfo.new(0.2),{
						TextTransparency = 0
					})
				else
	...
	function WindowArgs:DrawConfig(Configuration : TabConfigManager , Internal)
		Configuration = Fallens.__CONFIG(Configuration,{
			Name = "Config",
			Icon = "folder",
			Config = nil
		});
		...
		TabButton.Size = UDim2.new(1, -10, 0, 38)
		TabButton.ZIndex = 3
		TabButton:SetAttribute("SearchTag", Configuration.Name)
		...
		Icon.Size = UDim2.new(0, 15, 0, 15)
		Icon.ZIndex = 3
		Icon.Image = Fallens:_GetIcon(Configuration.Icon);
		Icon.ImageColor3 = Fallens.Colors.Highlight
		...
		TabConfig.Name = Fallens:_RandomString()
		...
		ConfigList.Name = Fallens:_RandomString()
		ConfigList.Parent = TabConfig
		ConfigList.AnchorPoint = Vector2.new(0.5, 0)
		ConfigList.BackgroundColor3 = Fallens.Colors.BlockColor
		...
		SectionText.Name = Fallens:_RandomString()
		...
		SectionText.TextColor3 = Fallens.Colors.SwitchColor
		...
		SectionClose.Name = Fallens:_RandomString()
		...
		SectionClose.Image = Fallens:CacheImage("rbxassetid://109535175596957")
		...
		UIStroke_2.Color = Fallens.Colors.StrokeColor
		...
		Header_2.Name = Fallens:_RandomString()
		...
		SectionText_2.Name = Fallens:_RandomString()
		...
		SectionText_2.TextColor3 = Fallens.Colors.SwitchColor
		...
		SectionClose_2.Name = Fallens:_RandomString()
		...
		SectionClose_2.Image = Fallens:CacheImage("rbxassetid://109535175596957")
		...
		UIStroke_3.Color = Fallens.Colors.StrokeColor
		...
		TextBox.Text = ""
		TextBox.TextColor3 = Fallens.Colors.SwitchColor
		...
		Button.BackgroundColor3 = Fallens.Colors.SwitchColor
		Button.BackgroundTransparency = 1.000
		Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Button.BorderSizePixel = 0
		...
		Frame_2.BackgroundColor3 = Fallens.Colors.Highlight
		...
		UIStroke_4.Color = Fallens.Colors.StrokeColor
		...
		Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
			...
			if Fallens.PerformanceMode then
				if TabConfig.Visible then
					Fallens:_SetNilP(TabConfig , TabMainFrame);
				else
					Fallens:_SetNilP(TabConfig , nil);
				end;
			else
				Fallens:_SetNilP(TabConfig , TabMainFrame);
			end;
		...
		local TabOpen = function(bool)
			if bool then
				...
				Fallens:_Animation(ConfigList,Tween,{
					BackgroundTransparency = 0,
				});

				Fallens:_Animation(AddConfig,Tween,{
					BackgroundTransparency = 0,
				});
				...
				Fallens:_Animation(SectionText,Tween,{
					TextTransparency = 0.5
				});
				...
				Fallens:_Animation(Frame_2,Tween,{
					BackgroundTransparency = 0.1,
				});
				...
				Fallens:_Animation(SectionText_2,Tween,{
					TextTransparency = 0.5
				});

				Fallens:_Animation(TextBox,Tween,{
					TextTransparency = 0
				});

				Fallens:_Animation(SectionClose,Tween,{
					ImageTransparency = 0.5,
				});

				Fallens:_Animation(SectionClose_2,Tween,{
					ImageTransparency = 0.5,
				});
			else
		...
		function TabArgs:_DrawConfig()
			...
			DelButton.Name = Fallens:_RandomString()
			...
			DelButton.ZIndex = 14
			DelButton.Image = Fallens:CacheImage("rbxassetid://10747362393")
			...
			ConfigBlock.Name = Fallens:_RandomString()
			...
			if Fallens:_IsMobile() then
				Fallens:_AddDragBlacklist(ConfigBlock);
			end;

			ConfigText.Name = Fallens:_RandomString()
			...
			ConfigText.TextColor3 = Fallens.Colors.SwitchColor
			...
			SaveButton.Name = Fallens:_RandomString()
			...
			Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
			Icon.ZIndex = 15
			Icon.Image = Fallens:CacheImage("rbxassetid://10734941499")
			Icon.ImageTransparency = 1;

			LoadButton.Name = Fallens:_RandomString()
			...
			TextLabel_2.TextColor3 = Fallens.Colors.SwitchColor
			...
			Icon_2.Name = Fallens:_RandomString()
			...
			Icon_2.ZIndex = 15
			Icon_2.Image = Fallens:CacheImage("rbxassetid://10723344270")
			...
			AuthorText.Name = Fallens:_RandomString()
			...
			AuthorText.TextColor3 = Fallens.Colors.SwitchColor
			...
			function ConfigButton:SetInfo(Author , ConfigName)
				local R,G,B = tostring(math.floor(Fallens.Colors.Highlight.R * 255)) , tostring(math.floor(Fallens.Colors.Highlight.G * 255)) , tostring(math.floor(Fallens.Colors.Highlight.B * 255));
			...
			function ConfigButton:Toggle(v)
				if v then
					Fallens:_Animation(ConfigBlock,Tween,{
						BackgroundTransparency = 0
					});

					Fallens:_Animation(LinkValues,Tween,{
						Position = UDim2.new(1, -12, 0.5, 0)
					});

					Fallens:_Animation(ConfigText,Tween,{
						TextTransparency = 0.3,
					...
					Fallens:_Animation(AuthorText,Tween,{
						TextTransparency = 0.5,
						Position = UDim2.new(0,AuthorText:GetAttribute('SPC'), 0.5, 0)
					});

					Fallens:_Animation(Icon_2,Tween,{
						ImageTransparency = 0
					});
					...
					Fallens:_Animation(TextLabel_2,Tween,{
						TextStrokeTransparency = 0.900,
						TextTransparency = 0
					});
				else
			...
			Fallens:_Input(LoadButton,function()
				task.spawn(ConfigButton.OnLoad);
			end);

			Fallens:_Input(SaveButton,function()
		...
		function TabArgs:Init()
			...
			Fallens:_Input(Button,function()
				if TextBox.Text:byte() then
					WindowArgs.Notify.new({
						Title = "Configs",
						Icon = Fallens:_GetIcon(Config.Logo),
			...
			local Refresh = function()
				...
				for i,v in next , FullConfig do
					...
					Button.OnLoad = function()
						WindowArgs.Notify.new({
							Title = "Configs",
							Icon = Fallens:CacheImage(Config.Logo),
	...
	function WindowArgs:DrawTab(TabConfig : TabConfig , Internal)
		...
		if Internal then
			...
			TabContent.Name = Fallens:_RandomString()
			...
			Internal.Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
				...
				if Fallens.PerformanceMode then
					if TabContent.Visible then
						Fallens:_SetNilP(TabContent , Internal.Parent);
					else
						Fallens:_SetNilP(TabContent , nil);
					end;
				else
					Fallens:_SetNilP(TabContent , Internal.Parent);
				end;
			...
			if Fallens:_IsMobile() then
				Fallens:_AddDragBlacklist(Left);
				Fallens:_AddDragBlacklist(Right);
			end;
			...
			Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
				...
				if Fallens.PerformanceMode then
					if TabContent.Visible then
						Fallens:_SetNilP(TabContent , TabMainFrame);
					else
						Fallens:_SetNilP(TabContent , nil);
					end;
				else
		...
		function TabArgs:DrawSection(config: Section)
			...
			Section.Name = Fallens:_RandomString()
			...
			Section.BackgroundColor3 = Fallens.Colors.BlockColor
			...
			if Fallens:_IsMobile() then
				Fallens:_AddDragBlacklist(Section);
			end;
			...
			local refresh = function(Upvalue)
				...
				if IsOpen then
					...
					if (Section:GetAttribute('Height') and not Fallens:_IsMobile() and FullScale <= RefPos) then
						Fallens:_Animation(Section,TweenInfo.new(0.4,Enum.EasingStyle.Quint),{
			...
			TabOpenSignal:Connect(function(bool)
				if bool then
					Fallens:_Animation(Section,TweenInfo.new(0.21),{
						BackgroundTransparency = 0
					})

					Fallens:_Animation(SectionText,TweenInfo.new(0.21),{
						TextTransparency = 0.500
					})

					Fallens:_Animation(SectionClose,TweenInfo.new(0.21),{
						ImageTransparency = 0.500
					})
				else
					Fallens:_Animation(Section,TweenInfo.new(0.21),{
			...
			Fallens:_Input(Header,function()
				IsOpen = not IsOpen;

				if IsOpen then
					Fallens:_Animation(SectionClose,TweenInfo.new(0.35),{
						Rotation = 0
					});
				else
					Fallens:_Animation(SectionClose,TweenInfo.new(0.35),{
			...
			Header.MouseEnter:Connect(function()
				Fallens:_Animation(SectionText,TweenInfo.new(0.2),{
			...
			return Fallens:_LoadElement(Section , true , TabOpenSignal)
		end;
	...
	do
		...
		function WindowArgs:Watermark()
			local Signal = Fallens.__SIGNAL(true);
			...
			Watermark.Name = Fallens:_RandomString()
			Watermark.Parent = FallensGui
			Watermark.AnchorPoint = Vector2.new(1, 0)
			Watermark.BackgroundColor3 = Fallens.Colors.BGDBColor

			Fallens:Drag(Watermark , Watermark, 0.1);
			...
			Logo.Name = Fallens:_RandomString()
			Logo.Parent = Watermark
			Logo.AnchorPoint = Vector2.new(1, 0.5)
			Logo.BackgroundColor3 = Fallens.Colors.BGDBColor
			...
			WaternarkList.Name = Fallens:_RandomString()
			...
			BackFrame.Name = Fallens:_RandomString()
			...
			Fallens:_Blur(BackFrame,Signal);

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				Fallens:_Animation(Watermark,TweenInfo.new(0.4),{
			...
			function Args:AddText(Watermark : Watermark)
				Watermark = Fallens.__CONFIG(Watermark, {
					Text = "Watermark",
					Icon = "info"
				});
				...
				Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
				Icon.ZIndex = 156
				Icon.Image = Fallens:_GetIcon(Watermark.Icon);
				...
				function Arg:Visible(v)
					...
					if Fallens.PerformanceMode then
						if v then
							Fallens:_SetNilP(Icon , WaternarkList);
							Fallens:_SetNilP(TextLabel , WaternarkList);
						else
							Fallens:_SetNilP(Icon , nil);
							Fallens:_SetNilP(TextLabel , nil);
						end;
					else
						Fallens:_SetNilP(Icon , WaternarkList);
						Fallens:_SetNilP(TextLabel , WaternarkList);
		...
		if not Fallens:_IsMobile() then
			...
			Fallens:_Hover(Button,function()
	...
	function WindowArgs:Update(config: WindowUpdate)
		...
		if Fallens.SecureMode and string.find(config.UserProfile, "rbxassetid://",1,true) then
			config.UserProfile = Fallens:_GetIcon("user");
		end;
	...
	WindowArgs.LOOP_THREAD = task.spawn(function()
		...
		BlurElement.Name = Fallens:_RandomString()
		...
		Fallens:_Blur(BlurElement , WindowOpen);

		local MovementFrame = Instance.new("Frame")

		MovementFrame.Name = Fallens:_RandomString()
		...
		Fallens:Drag(MovementFrame,MainFrame,0.1)
		...
		while true do task.wait(0.01);
			...
			if WindowArgs.SelectedTab and WindowArgs.IsOpen then
				...
				if vili < 0 or vili > TabButtons.AbsoluteSize.Y then
					Fallens:_Animation(SelectionFrame , TweenInfo.new(0.1) , {
						BackgroundTransparency = 1
					});
				else
					if math.abs(distance) <= 10 then
						...
						Fallens:_Animation(SelectionFrame , TweenInfo.new(0.15) , {
							BackgroundTransparency = 0,
	...
	Fallens:_Hover(TabFrame , function()
...
function Fallens:GetTimeNow(Time)
	Time = Time or tick();
...
function Fallens:GetConfig(Type: string)
	local ConfigFlags = {};
...
function Fallens:_Path(...)
	local args = {...};

	return table.concat(args, "/");
end;

function Fallens:ConfigManager(ConfigManager: ConfigManager) : ConfigFunctions
	ConfigManager = Fallens.__CONFIG(ConfigManager , {
		Directory = "Fallens",
		Config = "Software"
	});
	...
	if not isfolder(Fallens:_Path(ConfigManager.Directory , ConfigManager.Config)) then
		makefolder(Fallens:_Path(ConfigManager.Directory , ConfigManager.Config));
	end;

	local Args = {
		Directory = Fallens:_Path(ConfigManager.Directory , ConfigManager.Config);
		EnableNotify = false,
	};

	local notify = Fallens.newNotify();

	function Args:WriteConfig(Config: WriteConfig)
		Config = Fallens.__CONFIG(Config , {
			Name = Fallens:_RandomString(),
			Author = LocalPlayer.Name,
		});

		local Flags = Fallens:GetConfig("MK");

		Flags["__INFORMATION"] = {
			Type = "Information",
			Author = Config.Author,
			Name = Config.Name,
			CreatedDate = Fallens:GetDate()
		};

		if Args.EnableNotify then
			notify.new({
				Title = "Configs",
				Icon = Fallens:_GetIcon('settings'),
				Content = "Create config \""..Config.Name.."\""
			})
		end

		writefile(Fallens:_Path(Args.Directory , Config.Name) , HttpService:JSONEncode(Flags));
	end;

	function Args:LoadConfigFromString(str: string)
		local decoded = HttpService:JSONDecode(str);

		local Flags = Fallens:GetConfig("KV");
	...
	function Args:GetCurrentConfig()
		return Fallens:GetConfig("MK")
	end;

	function Args:ReadInfo(ConfigName: string)
		local _path = Fallens:_Path(Args.Directory , ConfigName);
...
function Fallens:Loader(IconId,Duration)
	local FallensGui = Instance.new("ScreenGui")

	FallensGui.Name = Fallens:_RandomString()
	FallensGui.Parent = CoreGui
	FallensGui.Enabled = true
	FallensGui.ResetOnSpawn = false
	FallensGui.IgnoreGuiInset = true
	FallensGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
	...
	Loader.Name = Fallens:_RandomString()
	Loader.Parent = FallensGui
	...
	Icon.ZIndex = 100
	Icon.Image = IconId or Fallens.Logo;
	Icon.ImageTransparency = 1

	Vignette.Name = Fallens:_RandomString()
	Vignette.Parent = Loader
	Vignette.BackgroundTransparency = 1.000
	Vignette.BorderColor3 = Color3.fromRGB(27, 42, 53)
	Vignette.BorderSizePixel = 0
	Vignette.Size = UDim2.new(1, 0, 1, 0)
	Vignette.Image = Fallens:CacheImage("rbxassetid://18720640102")
	Vignette.ImageColor3 = Fallens.Colors.Highlight
	Vignette.ImageTransparency = 1
	Vignette.AnchorPoint = Vector2.new(0.5,0.5)
	Vignette.Position = UDim2.fromScale(0.5,0.5)

	Fallens:_Animation(Loader,TweenInfo.new(0.55,Enum.EasingStyle.Quint),{
		BackgroundTransparency = 0.5
	});
	...
	task.delay(0.5,function()
		Fallens:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint),{
			ImageTransparency = 0.01,
			Size = UDim2.new(0, 200, 0, 200)
		});

		task.delay(0.25,function()
			Fallens:_Animation(Vignette,TweenInfo.new(5),{
				ImageTransparency = 0.2
			});

			task.wait(Duration or 4.5)

			Fallens:_Animation(Vignette,TweenInfo.new(3,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				Size = UDim2.new(2, 0, 2, 0)
			});

			Fallens:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				ImageTransparency = 1,
			});

			Fallens:_Animation(Loader,TweenInfo.new(1.5,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				BackgroundTransparency = 1
			});

			task.delay(0.1,function()
				Fallens:_Animation(Vignette,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
					ImageTransparency = 1
				});
...
function Fallens.newNotify()
	if Fallens.NOTIFY_CACHE then
		return Fallens.NOTIFY_CACHE;
	end;
	...
	Notification.Name = Fallens:_RandomString()
	Notification.Parent = CoreGui;
	Notification.ResetOnSpawn = false
	Notification.ZIndexBehavior = Enum.ZIndexBehavior.Global

	NotifyContainer.Name = Fallens:_RandomString()
	...
	Fallens.NOTIFY_CACHE = {
		new = function(Notify: Notify) : NotifyPayback
			Notify = Fallens.__CONFIG(Notify, {
				Icon = Fallens.Logo,
				Title = "Notification",
				Content = "Content",
				Duration = 3,
			...
			BlockFrame.Name = Fallens:_RandomString()
			BlockFrame.Parent = NotifyContainer
			BlockFrame.AnchorPoint = Vector2.new(1, 0)
			BlockFrame.BackgroundColor3 = Fallens.Colors.BGDBColor
			...
			NotifyFrame.Name = Fallens:_RandomString()
			NotifyFrame.Parent = BlockFrame
			NotifyFrame.BackgroundColor3 = Fallens.Colors.BGDBColor
			...
			if string.find(Notify.Icon,'cache-ck-',1,true) then	
				...
				CompLogo.Image = Fallens:_GetIcon(Notify.Icon);
			end;
			...
			Header.TextColor3 = Fallens.Colors.SwitchColor
			Header.TextSize = 14.000
			Header.TextXAlignment = Enum.TextXAlignment.Left

			Body.Name = Fallens:_RandomString()
			...
			Body.TextColor3 = Fallens.Colors.SwitchColor
			Body.TextSize = 12.000
			Body.TextTransparency = 0.500
			Body.TextXAlignment = Enum.TextXAlignment.Left
			Body.TextYAlignment = Enum.TextYAlignment.Top

			TimeLeftFrame.Name = Fallens:_RandomString()
			...
			TimeLeft.Name = Fallens:_RandomString()
			TimeLeft.Parent = TimeLeftFrame
			TimeLeft.BackgroundColor3 = Fallens.Colors.Highlight
			...
			local UpdateText = function()
				...
				if BlockFrame:GetAttribute('Already') then
					Fallens:_Animation(BlockFrame,TweenInfo.new(0.3),{
			...
			local Close = function()
				Fallens:_Animation(NotifyFrame,TweenInfo.new(0.65,Enum.EasingStyle.Quint),{
			...
			local Show = function()
				Fallens:_Animation(NotifyFrame,TweenInfo.new(0.5,Enum.EasingStyle.Quint),{
					Position = UDim2.new(0,0,0,0)
				});
			end;

			if typeof(Notify.Duration) == 'number' and Notify.Duration ~= math.huge then
				Fallens:_Animation(TimeLeft,TweenInfo.new(Notify.Duration + 0.2,Enum.EasingStyle.Linear),{
			...
			return {
				...
				SetProgress = function(self , new , Time)
					...
					Fallens:_Animation(TimeLeft,TweenInfo.new(Time or 0.85,(Time and Enum.EasingStyle.Linear) or Enum.EasingStyle.Quint),{
...
Fallens.NilFolder.Name = "Nil-Instances";
