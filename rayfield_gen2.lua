...
-- Menghitung scale otomatis berdasarkan ukuran layar (viewport) dan jenis device.
-- Layar kecil / mobile akan mendapat scale lebih kecil supaya UI tidak overflow,
-- sementara layar besar (PC) tetap di scale penuh (1).
...
function RayfieldLibrary:CreateWindow(Settings)
        ...
        local shownPosition = Settings.Position or UDim2.new(0.5, 0, 0.5, -math.floor(WINDOW_H / 2))
        -- Catatan: sebelumnya perhitungan tengah ikut menghitung +18px ruang untuk
        -- handle di bawah window, jadi window jadi sedikit naik dari titik tengah
        -- layar yang sebenarnya. Sekarang cuma pakai WINDOW_H supaya window (bukan
        -- root+handle) yang benar-benar center.
        --
        -- Kalau mau posisi custom, isi Settings.Position dengan UDim2. Root window
        -- ini AnchorPoint-nya (0.5, 0): X selalu di tengah, Y dihitung dari sisi
        -- ATAS window. Contoh:
        -- Position = UDim2.new(0.5, 0, 0, 40) -- tengah horizontal, nempel 40px dari atas layar
        -- Position = UDim2.new(0.5, 0, 1, -(WINDOW_H + 40)) -- tengah horizontal, nempel bawah layar
        ...
        -- UIScale: mengecilkan seluruh window (termasuk teks) secara proporsional,
        -- seperti zoom out. Bisa di-set manual lewat Settings.UIScale, kalau tidak
        -- diisi maka menyesuaikan otomatis berdasarkan ukuran layar/device.
        local manualScale = tonumber(Settings.UIScale)
        local uiScale = create("UIScale", {
                Scale = manualScale or getAutoScale(),
                Parent = root,
        })
        ...
        -- Bar visualnya cuma 130x4 px, jadi hit-area buat drag sangat kecil dan
        -- susah kena jari di mobile. HandleZone ini tidak terlihat (transparan)
        -- tapi jauh lebih besar, jadi menangkap sentuhan/klik di sekitar handle
        -- tanpa mengubah tampilan bar aslinya.
        local handleZone = create("Frame", {
                Name = "HandleZone",
                AnchorPoint = Vector2.new(0.5, 0.5),
                Position = UDim2.new(0.5, 0, 0, WINDOW_H + 12 + 2),
        ...
        connect(window:GetPropertyChangedSignal("Size"), function()
                ...
                handleZone.Position = UDim2.new(0.5, 0, 0, size.Y.Offset + 12 + 2)
        ...
        local function buildTabAPI(page, compact)
                ...
                function Tab:CreateSlider(SliderSettings)
                        ...
                        local columns = SliderSettings.Columns or 1
                        ...
                        card:SetAttribute("Columns", columns)
                        paint(card, "BackgroundColor3", "Card")
                        cardBase(card)
            ...
            create("UIGradient", {
                ...
                function Tab:CreateInput(InputSettings)
                        ...
                        card:SetAttribute("Columns", InputSettings.Columns or 1)
                ...
                function Tab:CreateDropdown(DropdownSettings)
                        ...
                        wrapper:SetAttribute("Columns", DropdownSettings.Columns or 1)
                ...
                function Tab:CreateKeybind(KeybindSettings)
                        ...
                        card:SetAttribute("Columns", KeybindSettings.Columns or 1)
                ...
                function Tab:CreateColorPicker(ColorPickerSettings)
                        ...
                        card:SetAttribute("Columns", ColorPickerSettings.Columns or 1)
                        paint(card, "BackgroundColor3", "Card")
                        cardBase(card)
                        hoverable(card)
                ...
                function Tab:CreateRow()
                        ...
                        local function recompute()
                                ...
                                for _, c in ipairs(rowFrame:GetChildren()) do
                                        if c:IsA("GuiObject") and not c:IsA("UIListLayout") then table.insert(kids, c) end
                                end
                                local n = #kids
                                if n == 0 then return end
                                local totalCols = 0
                                for _, c in ipairs(kids) do
                                        totalCols = totalCols + (c:GetAttribute("Columns") or 1)
                                end
                                if totalCols == 0 then return end
                                local gap = 8
                                local totalGap = gap * math.max(n - 1, 0)
                                local allocated = 0
                                for i, c in ipairs(kids) do
                                        local cols = c:GetAttribute("Columns") or 1
                                        local fraction = cols / totalCols
                                        local adj
                                        if i < n then
                                                adj = math.floor(totalGap * fraction + 0.5)
                                                allocated = allocated + adj
                                        else
                                                adj = totalGap - allocated
                                        end
                                        c.Size = UDim2.new(fraction, -adj, 0, c.Size.Y.Offset)
                                end
                        end
                ...
                function Tab:CreateColumns(count)
                        ...
                        local gap = 10
                        local function recomputeColumns()
                                local n = #columns
                                local totalWeight = 0
                                local weights = {}
                                for _, col in ipairs(columns) do
                                        ...
                                        for _, child in ipairs(col:GetChildren()) do
                                                if child:IsA("GuiObject") and not child:IsA("UIListLayout") then
                                                        local c = child:GetAttribute("Columns") or 1
                                                        if c > maxCols then maxCols = c end
                                                end
                                        end
                                        weights[col] = maxCols
                                        totalWeight = totalWeight + maxCols
                                end
                                if totalWeight == 0 then return end
                                local totalGap = gap * math.max(n - 1, 0)
                                local allocated = 0
                                for i, col in ipairs(columns) do
                                        local w = weights[col] / totalWeight
                                        local adj
                                        if i < n then
                                                adj = math.floor(totalGap * w + 0.5)
                                                allocated = allocated + adj
                                        else
                                                adj = totalGap - allocated
                                        end
                                        col.Size = UDim2.new(w, -adj, 0, 0)
                                end
                        end
                        for _, col in ipairs(columns) do
                                col.ChildAdded:Connect(function()
                                        task.defer(recomputeColumns)
                                end)
                        end
                        recomputeColumns()
        ...
        local function makeDraggable(zone, allowWhenHidden)
                local dragging = false
                local dragStart = nil
                local startPos=nil
                zone.InputBegan:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                                if morphing or (hidden and not allowWhenHidden) then return end
                                dragging = true
                ...
                connect(UserInputService.InputChanged, function(input)
                        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                                ...
                                -- Set posisi langsung (tanpa tween). Sebelumnya setiap event drag
                                -- membuat Tween baru berdurasi 0.3s, jadi banyak tween saling
                                -- menumpuk/menimpa dan hasilnya patah-patah di mobile. Assignment
                                -- langsung membuat window mengikuti jari 1:1, jauh lebih mulus.
        ...
        -- Dragging untuk pill/button "open menu" (muncul saat window disembunyikan).
        -- Dipisah dari makeDraggable karena tombol ini juga harus tetap bisa di-tap
        -- untuk membuka window, jadi perlu dibedakan mana "geser" dan mana "tap".
        do
                ...
                connect(UserInputService.InputChanged, function(input)
                        if pillDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                                ...
                                if not pillDragged and delta.Magnitude > DRAG_THRESHOLD then
                                        pillDragged = true
                                end
                                root.Position = UDim2.new(
        ...
        function Window:SetUIScale(scale)
                if type(scale) == "number" then
                        uiScale.Scale = math.clamp(scale, 0.4, 2)
                end
        end
