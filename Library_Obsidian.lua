...
function Fallens:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal)
	if Name == "Toggle" then
		...
		ToggleGradient.Name = Fallens:_RandomString()
...
function Fallens:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal , DisableStackKeybind)
	...
	function Args:AddSlider(Config: Slider)
		...
		SliderGradient.Name = Fallens:_RandomString()
		SliderGradient.Parent = SliderInput
...
function Fallens.new(Config : Window)
	...
	MainFrameStroke.Color = Fallens.Colors.StrokeColor
	MainFrameStroke.Thickness = 1
	MainFrameStroke.Transparency = 0.2

	table.insert(Fallens.Elements.StrokeColor,{
		Element = MainFrameStroke,
		Property = "Color"
	});
	...
	local SubWindowLabel = Instance.new("TextLabel")

	SubWindowLabel.Name = Fallens:_RandomString()
	SubWindowLabel.Parent = TabFrame
	SubWindowLabel.BackgroundTransparency = 1.000
	SubWindowLabel.BorderColor3 = Color3.fromRGB(0, 0, 0)
	SubWindowLabel.BorderSizePixel = 0
	SubWindowLabel.Position = UDim2.new(0, 60, 0, 31)
	SubWindowLabel.Size = UDim2.new(0, 200, 0, 14)
	SubWindowLabel.Font = Enum.Font.Gotham
	SubWindowLabel.Text = Config.SubName
	SubWindowLabel.TextColor3 = Fallens.Colors.Highlight
	SubWindowLabel.TextTransparency = 0.5
	SubWindowLabel.TextSize = math.max(Config.TextSize - 4, 10)
	SubWindowLabel.TextXAlignment = Enum.TextXAlignment.Left
	SubWindowLabel.ClipsDescendants = true;

	table.insert(Fallens.Elements.Highlight , {
		Element = SubWindowLabel,
		Property = 'TextColor3'
	});

	local function _AttachGlowSweep(Label: TextLabel, Duration: number, BaseTransparency: number)
		local GlowLabel = Instance.new("TextLabel")
		GlowLabel.Name = Fallens:_RandomString()
		GlowLabel.Parent = Label
		GlowLabel.AnchorPoint = Vector2.new(0, 0)
		GlowLabel.Position = UDim2.new(0, 0, 0, 0)
		GlowLabel.Size = UDim2.new(1, 0, 1, 0)
		GlowLabel.BackgroundTransparency = 1
		GlowLabel.BorderSizePixel = 0
		GlowLabel.Font = Label.Font
		GlowLabel.TextSize = Label.TextSize
		GlowLabel.TextXAlignment = Label.TextXAlignment
		GlowLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
		GlowLabel.TextTransparency = 0
		GlowLabel.ZIndex = Label.ZIndex + 1

		local Gradient = Instance.new("UIGradient")
		Gradient.Name = Fallens:_RandomString()
		Gradient.Parent = GlowLabel
		Gradient.Color = ColorSequence.new(Color3.fromRGB(255, 255, 255))
		Gradient.Transparency = NumberSequence.new({
			NumberSequenceKeypoint.new(0, 1),
			NumberSequenceKeypoint.new(0.42, 1),
			NumberSequenceKeypoint.new(0.5, BaseTransparency),
			NumberSequenceKeypoint.new(0.58, 1),
			NumberSequenceKeypoint.new(1, 1),
		})
		Gradient.Offset = Vector2.new(-1.5, 0)

		local GlowTween = TweenService:Create(Gradient,TweenInfo.new(Duration,Enum.EasingStyle.Linear,Enum.EasingDirection.InOut,-1,false,0.4),{
		...
		Label:GetPropertyChangedSignal("Text"):Connect(UpdateText);
	end;

	_AttachGlowSweep(WindowLabel, 2.6, 0)
	_AttachGlowSweep(SubWindowLabel, 2.6, 0.35)
	...
	WindowOpen:Connect(function(v)
		...
		if v then
			...
			CompLogo.ImageTransparency = 0

			WindowLabel.TextTransparency = 0

			SubWindowLabel.TextTransparency = 0.5

			UserProfile.ImageTransparency = 0

			UserText.TextTransparency = 0

			ExpireText.TextTransparency = 0.5

			MainFrame.BackgroundTransparency = 0

			LineFrame1.BackgroundTransparency = 0
			LineFrame1.Size = UDim2.new(0, 20, 1, 0)

			TabFrame.BackgroundTransparency = TabFrameBaseTrans
		else
			MainFrame.Size = UDim2.new(math.max(Config.Scale.X.Scale - 0.05,0) , Config.Scale.X.Offset - 10 , math.max(Config.Scale.Y.Scale - 0.05,0) , Config.Scale.Y.Offset - 10)

			TabButtonScrollingFrame.Position = UDim2.new(1.5, 100, 0.5, 0)

			LineFrame1.BackgroundTransparency = 1
			LineFrame1.Size = UDim2.new(0, 1, 1, 0)

			CompLogo.ImageTransparency = 1

			WindowLabel.TextTransparency = 1

			SubWindowLabel.TextTransparency = 1

			UserProfile.ImageTransparency = 1

			UserText.TextTransparency = 1

			ExpireText.TextTransparency = 1
	...
	function WindowArgs:DrawContainerTab(TabConfig : ContainerTab)
		...
		TabNameLabel.TextColor3 = Fallens.Colors.Highlight
	...
	do
		...
		TitleLabel.Name = Fallens:_RandomString()
		TitleLabel.Parent = CloseWindow
		...
		SubtitleLabel.Name = Fallens:_RandomString()
		SubtitleLabel.Parent = CloseWindow
		...
		local ToggleCloseUI = function(v)
			...
			if v then
				...
				Fallens:_Animation(TitleLabel,TweenInfo.new(0.2),{
					TextTransparency = 0.1
				})

				Fallens:_Animation(SubtitleLabel,TweenInfo.new(0.2),{
